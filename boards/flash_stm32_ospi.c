			     ...
			     const uint8_t *data, size_t size)
{
	...
	LOG_DBG("PP cmd=0x%x instrMode=%d addrMode=%d addrSize=%d dataMode=%d addr=0x%x nb=%d",
	cmd->Instruction, cmd->InstructionMode, cmd->AddressMode,
	cmd->AddressSize, cmd->DataMode, cmd->Address, cmd->NbData);
...
static int stm32_ospi_set_memorymap(const struct device *dev)
{
	...
	/* Invalidate ICACHE after indirect writes so MemoryMap reads are not stale */
				  ...
				  const void *data, size_t size)
{
	...
	LOG_DBG("OSPI: write %zx data, address %zx", size, addr);
	LOG_HEXDUMP_INF(data, size, "Date for write");
...
end_write:
	...
	uint8_t new_data[1024];
	LOG_DBG("OSPI: check read %zx data, addr: %zx", tmp_size, tmp_addr);
	int err = flash_stm32_ospi_read(dev, tmp_addr, new_data, tmp_size);
	if (err == 0) {
		LOG_INF("read ok: addr=0x%lx size=%zx", (long)tmp_addr, tmp_size);
		LOG_HEXDUMP_INF(new_data, tmp_size, "Readed_data");
		//LOG_HEXDUMP_INF(tmp_data, tmp_size, "temp_data");
	} else {
			       ...
			       const struct jesd216_bfp *bfp)
{
	...
	if (data->write_opcode == SPI_NOR_WRITEOC_NONE) {
		...
		case OSPI_OPI_MODE:
			...
			break;
		case OSPI_QUAD_MODE:
			data->write_opcode = SPI_NOR_CMD_PP_1_1_4;
			break;
		case OSPI_DUAL_MODE:
