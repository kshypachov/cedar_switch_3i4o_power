...
nine edits in the table below live in the `zephyr` checkout, which this product
...
| FRAM MB85RS64V accepted by the `fujitsu,mb85rsxx` driver, with a 2-byte address for parts up to 512 Kbit | `zephyr/drivers/eeprom/eeprom_mb85rsxx.c` | `west patch apply` of that one entry, see below | The upstream driver accepts the MB85RS64V and sizes the address from the part |
...
patches/
  ...
  zephyr/
    ...
    eeprom_mb85rsxx-support-mb85rs64v.patch
  workspace/
