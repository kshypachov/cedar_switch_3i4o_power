...
- `POST /firmware/uploads` с `target="stm32u585"` — тот же поток, что у ESP32 (кусок → задача → `received_bytes`), тот же ресурс `Upload` с `target`. Без `target` — `esp32c6`, как до этапа. **Одна активная загрузка на оба target**: пока есть неудачная или незавершённая загрузка другого target — `409 busy`. `GET /firmware/uploads` (с 2026-09-19) → `{uploads: [Upload]}` — загрузка каждого хранилища в любом состоянии, не больше одной на target: так страница находит загрузку, начатую не ею (другой браузер, скрипт, потерянный id), чтобы продолжить, проверить, установить или удалить её; `409 busy` называет id мешающей загрузки.
