...
> **Состояние на 2026-09-13.** P0 частично, P1 закрыт, **P2 закрыт** — локально,
> на плате и в CI на GitHub (run 34745868566 на `82601a6`, все три задачи). Готовы: `web-auth`,
> `web-api` (с адаптером HTTP-сервера), `web-assets`, приложение на React с
> экранами setup, входа, доступа и обзора; весь legacy HTTP удалён. Проверки:
> sim — 277 тестов: 129 в сюитах P1 (с тестами новых функций) и 148 в четырёх
> новых; контракт и mock — пять из
> пяти проверок документа и набор pytest; фронтенд — 109 unit/component, 5 e2e
> против mock и 1 против платы; наборы проверены мутациями (счёт — в
> `reports/p2`). На плате (`reports/p2/hw`): IPv4 и IPv6, legacy закрыт, setup,
> вход, 429, смена пароля, сохранение после сброса, 2 и 4 клиента, abort,
> латентность. Отметки `~~зачёркнуто~~` и колонки «Статус» в разделах 3, 10 и 12
> показывают, что сделано.
>
> Сетевые, Matter, логи и ESP32 модули P1 по-прежнему не работали на плате:
> их API — этапы P3–P6.
...
|---|---|---|
| HTTP | ~~`src/web/http_server_init.c`: порт 80, 4 клиента; обработчик неизвестных URL перенаправляет на `/`~~ **P2:** `src/web/web_server.c` — один сервис на `[::]:80` (IPv4 и IPv6), ресурс на путь API, fallback: JSON 404 для `/api/`, остальное — `web-assets` | ~~Разделить SPA fallback и JSON 404; единый API v1, контексты запросов, обработка abort, лимиты~~ — сделано в `web-api` и `web-assets`. Остаётся: поток сервера кооперативный и один — долгие обработчики недопустимы (раздел 9, KDF) |
| Фронтенд | ~~`.gitmodules`: отдельный `src/web/frontend`, gitlink `ec332cb…`; каталог пуст~~ **P2:** приложение с нуля в `src/web/frontend` (React, TypeScript, Vite) | ~~Решением владельца не используется: удалить запись submodule, написать приложение с нуля под API v1~~ — сделано: экраны setup, входа, доступа, обзора |
| Сборка UI | ~~Корневой CMake перечисляет `dist` при конфигурации; `frontend_build` вызывает `npm install`~~ **P2:** `npm ci` по lockfile; firmware встраивает `dist` генератором `tools/web-assets` на этапе build, с бюджетом 512 КиБ; без собранного фронтенда сборка останавливается или встраивает заглушку (`APP_WEB_UI_PLACEHOLDER`) | ~~Переписать под новый фронтенд~~ — сделано |
| Авторизация | ~~`authentication.c` и login в основном закомментированы, init выключен~~ **P2:** удалены; `web-auth` — один администратор, PBKDF2-verifier в settings-registry, сессии с CSRF, лимит попыток | ~~Новый завершённый session flow, единая проверка для всех маршрутов~~ — сделано; legacy-маршруты (`/upload` на 8080, `/api/*`) удалены решением владельца |
...
| Обновление STM32 | ~~`/upload` пишет `slot1_partition` и помечает MCUboot pending~~ (удалён в P2). **Этап «Обновление STM32», 2026-09-15** (`reports/stm32-update`): загрузка через API v1 сразу в слот 2, `POST /system/updates`, обмен MCUboot, самоподтверждение через 20 минут | Это другой target: не использовать как ESP32 updater |
...
| ~~HTTP только на IPv4~~ | ~~`net conn` на плате показывает `0.0.0.0:80` и отсутствие `[::]:80`~~ **P2, решение владельца:** сервис на `[::]:80` с IPv4-mapped; на плате отвечает по IPv4 и IPv6 link-local | — |
...
| `job-manager` | Идентификаторы задач, состояния, прогресс, дедупликация, ограниченная история | Конкретные драйверы | **готов** — 22 теста (17 в P1, 4 в P2, переход `waiting_confirmation → running` в P4), 6 мутаций |
| `network-manager` | `get_status`, `scan`, `stage_config`, `apply`, `confirm`, `rollback`; Ethernet/Wi-Fi adapters | JSON и UI | **готов для P4** — 96 тестов, 66 мутаций пойманы (1 эквивалентна); адаптеры W5500 и ESP-Hosted — `src/services/network` (13 тестов чистых частей), Ethernet проверен на плате B, Wi-Fi — до P6 |
...
| `log-store` | Ограниченные кольцевые буферы, cursor, потери, фильтрация, экспорт | Сетевые отправки из log callback | **готов для P5** — два кольца в PSRAM по 256 КиБ, глобальный seq, cursor с тегом boot и фильтров, чтение под lock на одну запись; 43 теста в двух конфигурациях (кольца 4 и 256 КиБ), 40 мутаций (3 эквивалентны); на плате — см. `reports/p5/hw` |
| `zephyr-log-source` | Zephyr log backend → log-store; ранние сообщения по возможности | Долгое форматирование/flash I/O в ISR | **готов для P5** — backend deferred, текст без префикса, потери счётчиком, паника без lock; ранние сообщения приходят из deferred-буфера ядра лога (кольцо готово до старта потоков); 14 тестов, 17 мутаций; обход ошибки ядра лога `LOG_FAILURE_REPORT_PERIOD=0` |
| `coprocessor-manager` | Состояние C6, transport health, EN/BOOT, эксклюзивное владение USART3, режим UART | HTTP и формат пакета прошивки | **готов для P5** (без updater) — автомат `console/usb_bridge/flashing/unavailable`, остановка RX с таймаутом, маркеры и generation, claim-then-check с network-manager; 42 теста, 56 мутаций (2 эквивалентны, 2 гонки не воспроизводятся в sim); адаптер платы — `src/services/coprocessor` (собственный passthrough на `cdc_acm_uart0`, узел `uart-bridge0` выключен) |
...
| `esp32-log-source` | UART bytes → строки/records, reconnect/reset markers, ANSI sanitization | Второй UART callback рядом с flasher/bridge | **готов для P5** — сборка строк 512 Б, ANSI, UTF-8 → U+FFFD, разбор ESP-IDF, баннер ROM в любом месте строки; 33 теста на синтетике и ROM-потоке платы B, 28 мутаций; реальный вывод C6 с уровнями ESP-IDF — до P6 |
...
| `system-image-store` (этап «Обновление STM32») | Приём образа STM32 кусками сразу в слот 2 MCUboot, стирание секторов, чтение обратно, потоковая проверка формата MCUboot и SHA-256 TLV, метаданные в `/lfs/firmware` | Трейлер MCUboot, решение об обмене, HTTP | **начат** — 76 sim-тестов на симуляторе flash, мутаций 161 (149 убиты); на плате A — приём и проверка 1,4 МБ (`reports/stm32-update`) |
| `system-updater` (этап «Обновление STM32») | Автомат установки `preparing → requesting → rebooting`, журнал, сверка после старта, откат `rolled_back`, самоподтверждение через 20 минут, `confirm_now` | Запись flash и трейлера (платформа), приём образа | **начат** — 64 sim-теста на фейковой платформе, мутаций 158 (155 убиты, 3 эквивалентны); на плате A — обмен, самоподтверждение, откат |
| `src/services/system` (адаптер) | bootutil (`boot_set_pending/confirmed`), подтверждение через XIP-драйвер только с SRAM-стека, IWDG, причина сброса, шелл `sysupd` | Решения об обновлении | **начат** — проверен на плате A (шаг 0 и приёмка) |
| `web-auth` | Локальный admin, login/logout/password, cookie/CSRF, bounded sessions | Пользователи Matter fabrics | **готов** — 48 sim-тестов; на плате: setup, вход, 429, смена пароля, сохранение после сброса |
| `api-validation` (добавлен) | Коды ошибок и их HTTP-статусы, тело ошибки, строгие парсеры адресов и идентификаторов | Политика: подсеть, достижимость, состояние | **готов** — 24 теста (экранирование строки добавлено в P2), 12 мутаций |
| `web-api` | HTTP маршрутизация, JSON schemas, auth middleware, request contexts, вызовы сервисов | Бизнес-логика и ожидание завершения операций | **готов для P5** — 160 sim-тестов в `web_api` и `web_api_http` (ядро и адаптер на настоящем сервере Zephyr); обслуживает 28 из 35 операций (P5 добавил 4: логи и статус копроцессора), остальные — P6; в P4 — флаг `WEB_JSON_ONEOF`, в P5 — потоковый ответ, чтение query, откат JSON writer, `SO_SNDTIMEO` клиента |
| `web-assets` | Встраивание dist, MIME, gzip, ETag, index cache policy | Сетевые настройки и пароли | **готов** — 9 sim-тестов, генератор 14 pytest; на плате отдаёт приложение |
...
~~Стек выбирается в P2 по трём ограничениям~~ **Выбран в P2** (владелец оставил выбор исполнителю): React 19, TypeScript 5.9, Vite 8, vitest + Testing Library, Playwright. Ограничения выполнены и проверяются: итоговый `dist` — 80 164 байт gzip из 512 КиБ (15 %), проверка в сборке фронтенда и в сборке firmware; сборка по lockfile через `npm ci`; без CDN и внешних шрифтов — страница отдаётся с CSP `default-src 'self'`, e2e падает на любом запросе за пределы origin. Подробности — `src/web/frontend/README.md`. Типы API client генерируются из `openapi.json`; ручное дублирование схем в коде запрещено, расхождение ловится сборкой. QR-код строится в браузере из `qr_payload`, без внешнего сервиса. Никакой аналитики, телеметрии и внешних запросов: устройство работает в изолированной сети.
...
- Не выдавать приватные ключи, сертификатные секреты и pairing credentials в общие логи. Штатный код (сервис, API) onboarding codes в лог не пишет; отладочные команды шелла Matter, которые печатают QR и ручной код, остаются — решение владельца 2026-09-13 (раздел 13).
...
**Реализовано в P2** (`modules/web-auth/README.md`). Setup token показывается в веб-интерфейсе (решение владельца) — физический доступ он не доказывает, защита в момент первого включения — доверенная сеть, проверка Host и Origin. Пароль хранится как PBKDF2-HMAC-SHA256 со случайной солью. **Параметры подобраны измерением:** поток HTTP-сервера Zephyr кооперативный и единственный, KDF в нём останавливал всю систему (>18 с при 10000 итераций); теперь вывод идёт на отдельном вытесняемом потоке, сеть, Matter и шелл при этом работают, а HTTP-сервер ждёт. На плате ≈0,31 мс на итерацию; выбрано 3000 — вход ≈1,1 с. Попытки ограничены: 5 неудач с адреса бесплатно, дальше удвоение ожидания до 300 с, и не больше 30 неудач со всех адресов за 5 минут. Восстановления забытого или повреждённого пароля в первой версии нет — открытый вопрос владельцу (раздел 13).
...
| HTTP status reads | p95 < 250 мс в LAN без packet loss; отдельно нагрузка обновления и логов. Базовый замер 2026-09-12 после снятия флуда: HTTP p50 53 мс, p95 56 мс, ping 8,5–11,6 мс без потерь. **P2, на плате** (метод P0, `reports/p2`): `/api/v1/system/status` p50 98 / p95 103 мс; одно keep-alive соединение p95 76 мс; 2 клиента p95 157 мс; **4 клиента p95 313 мс — выше цели**, сервер обслуживает запросы по одному; страница `/` p50 125 / p95 137 мс против 53 / 56 у legacy-статики |
| UI assets | Целевой gzip budget ≤ 512 КиБ. **P2:** 80 373 байт в образе (4 файла), FLASH приложения 27,7 % → 29,6 % |
...
| P2. Web shell и доступ | ~~Выбор стека и сборка с нуля~~, ~~генерация типов из OpenAPI~~, ~~assets/ETag/gzip~~, ~~login/setup/password/logout~~, ~~общий middleware~~, ~~экран обзора~~ | UI работает офлайн, 2 клиента, нет обхода авторизации через legacy, корректная уборка при abort | P1 | 7–10 | ✅ **закрыт** (`reports/p2/README.md`). CI на GitHub, run 34745868566 на `82601a6`: sim 277 из 277 в 8 конфигурациях, контракт 5 из 5 проверок + 321 + 14 pytest, фронтенд 109 vitest, сборка и 5 e2e против mock (e2e против платы в CI пропускается). Критерий на плате: ~~UI офлайн~~ — e2e против платы без внешних запросов и нарушений CSP; ~~2 клиента~~ — 2 браузера (e2e), 2 клиента p95 157 мс без ошибок; ~~нет обхода через legacy~~ — весь legacy HTTP удалён решением владельца, пути отвечают JSON 404, порт 8080 закрыт; ~~уборка при abort~~ — контекст освобождается по ABORTED (sim на настоящем сервере и плата), зависший клиент держит только свой путь 10 с. Также: ~~пятая контрактная проверка~~ — таблица маршрутов устройства сверяется с документом. Не сделано/открыто: 4 клиента p95 313 мс; гипотеза о цене заголовков в сервере Zephyr не подтверждена захватом; восстановление пароля — вопрос владельцу |
...
| P4. Сеть | Ethernet/Wi-Fi adapters, scan, reconnect, DHCP/static/DNS, откат транзакции, web-экран | Все сценарии apply/confirm/timeout/power-loss, достижимость Matter при Wi-Fi-only | P1–P2 | 6–9 | 🟡 **не закрыт** (`reports/p4/README.md`), вёлся на плате B. Сделано: ~~адаптеры W5500 и ESP-Hosted~~, ~~DHCP из `net_init.c` в сервис~~, ~~DNS авто/вручную~~, ~~9 операций API~~, ~~экран «Сеть»~~, ~~scan (64, `truncated`, запрет при apply)~~, ~~правило пяти стартов~~; sim 401/401, контракт 5/5 + 340 pytest, фронтенд 222 + 13 e2e, 161 мутация без выживших. На плате B: ~~DHCP↔static с confirm по новому IP~~, ~~таймаут и откат по запросу~~, ~~DNS~~, ~~перезагрузка в `staged` и `awaiting_confirmation`~~, ~~Matter-mDNS после смен~~, ~~e2e~~. Найдено и исправлено на плате: переполнение стека `main`, неудаляемый ручной DNS, патч W5500 v1. Также ~~снятие кабеля~~, ~~потеря питания в `awaiting_confirmation`~~, ~~пять коротких стартов~~ (сбросом через J-Link по просьбе владельца). **Не выполнено:** Wi-Fi и Matter при Wi-Fi-only — до P6 (C6 платы B без прошивки, решение владельца) |
| P5. Логи и владелец UART | STM32 backend, C6 RX source, ring/cursors/export, контролируемый USB bridge, web-экран | Два источника, overflow и gaps, залипший браузер не блокирует логгер, переключение владельца UART | P1–P2 | 4–6 | 🟡 **сделан, не закрыт** (`reports/p5/README.md`), плата B. Сделано: ~~`log-store`~~, ~~`zephyr-log-source`~~, ~~`esp32-log-source`~~, ~~`coprocessor-manager`~~ с крючками network-manager, ~~потоковый экспорт и `SO_SNDTIMEO`~~, ~~4 операции API~~ (28 из 35), ~~экран «Логи»~~, ~~mock~~; sim 623/623, контракт 5/5 + 359, фронтенд 264 + 18 e2e, мутации 314 (без выживших, кроме обоснованных). На плате B: ~~два источника~~ (порядок seq, маркеры), ~~overflow~~ (точная сверка всплеска 402 + 1 098 = 1 500), ~~gap и wrap~~, ~~cursor через сброс~~ (новый boot_id, gap), ~~ранние сообщения~~ (seq 1 на 1,65 с), ~~медленный и зависший клиент~~ (другой клиент ждёт 6,9 с — таймаут отправки), ~~переключение console ↔ usb_bridge ↔ flashing~~ (10–100 мс, DTR 0,6/1,4 с), ~~сброс C6 без BUS FAULT~~, ~~e2e~~. **Открыто:** экспорт держит поток HTTP на всё время отдачи (28,7 с на 2 000 записей); голодание вытесняемых потоков после потока через мост к хосту (3 из 4 прогонов `uart`, причина не установлена); вывод C6 с уровнями ESP-IDF — до P6 |
...
| Обновление STM32 (вне нумерации: P7 — SoftAP) | Загрузка образа MCUboot через веб сразу в слот 2, проверка заголовка и TLV, установка через обмен MCUboot, журнал и сверка после старта, самоподтверждение через 20 минут записью в XIP, IWDG, экран «Прошивка» | Загрузка → обмен → подтверждение → сохранение после сброса; откат при сбросе до подтверждения и при зависании | P2, P6 (upload) | — | 🟡 **начат 2026-09-15** (`reports/stm32-update/README.md`), worktree `cedar_p7`, плата A. Сделано: ~~`system-image-store`~~ (sim 76, мутации 149/161), ~~`system-updater`~~ (sim 64, 155/158), ~~2 операции API, `target` у загрузок~~ (37 из 37), ~~mock~~ (464 pytest), ~~экран~~ (vitest 455, e2e 31), ~~sim привязок~~ (21). На плате A: ~~обмен, подтверждение через XIP, откат по IWDG~~ (шелл), ~~загрузка, проверка и установка через API, самоподтверждение через 20 мин и сохранение после сброса~~. Открыто: откат по сбросу до подтверждения через API, понижение версии, обрыв питания, скорость приёма (7,5 КиБ/с) и проверки (62,7 с), экран против платы |
...
| Обновление STM32 | Загрузка в слот 2 и проверка, чужой/битый образ, понижение версии без и с флажком, обмен MCUboot, самоподтверждение и подтверждение из консоли, сброс до подтверждения → откат, зависание → IWDG → откат, потеря питания посреди приёма и обмена, запрет при неподтверждённой прошивке |
...
**HTTP и Авторизация — измерено в P2, 2026-09-13** (`reports/p2`, образ `a77dee0c…`). Fragmented JSON, unknown field, oversized body (413 без буферизации), abort, 2 клиента, повторное использование контекста — sim на настоящем сервере Zephyr (`tests/web_api_http`) и плата. Login/logout/expiry/reboot, неверный пароль и 429, CSRF и Origin, чужой Host, legacy routes (JSON 404, 8080 закрыт), cookie без `Secure` — плата и e2e. Открыто: memory exhaustion не испытывался; секреты в логах проверены только чтением кода (`web_auth.c` не логирует пароль, токены и verifier).

**Ethernet и Apply — измерено в P4, 2026-09-13/14** (`reports/p4`, плата B, образы `2d39a97f…` и `0d32d454…`). Ethernet: ~~DHCP/static~~ (confirm по новому IP, новый адрес через 2,7–2,8 с), ~~invalid netmask/gateway~~ (12 отказов с полями как у mock), ~~DNS~~ (вручную и автоматически), ~~IPv6/mDNS~~ (`_matterc._udp` платы B с link-local и SLAAC после смен адреса). ~~Cable removal~~ (тот же адрес после возврата, Matter не пересоздан). Открыто: route change между интерфейсами (нужен Wi-Fi). Apply: ~~retry с тем же ключом~~, ~~два клиента с разными revisions~~ (`409 stale_revision`), ~~confirm по новому IP~~, ~~timeout rollback~~ (60,3 с), ~~reboot~~ в `staged` (404) и `awaiting_confirmation` (откат на старте, 410). ~~Потеря питания~~ в `awaiting_confirmation` (откат на старте, 410). Открыто: смена обоих интерфейсов (нужен Wi-Fi), ошибка записи settings на плате (в sim — да). Wi-Fi — вся строка до P6 по решению владельца. Время записи конфигурации на файловом бэкенде растёт по ходу сессии (apply 1,1 → 10 с) — измерено, решение в другом треде.

**Logs — измерено в P5, 2026-09-14** (`reports/p5`, плата B, образы `9bc06ceb…`, `cc933cfd…`, `2e2f30b5…`). ~~Burst~~ (всплеск STM32 `coproc burst`: в кольце 402, `dropped_count` +1 098 — ровно 1 500; ROM-флуд C6 11 КБ/с), ~~длинная строка~~ и ~~invalid UTF-8/ANSI~~ (sim на синтетике и фикстуре, на плате — строки U+FFFD ROM), ~~binary UART ROM data~~ (поток C6 без прошивки, баннеры посреди строк), ~~ring wrap~~ (кольцо ESP32 оборачивается за ~20 с, `gap=true`), ~~старый cursor~~ (400 для повреждённого и чужого, gap для перезаписанного), ~~boot change~~ (новый `boot_id`, `gap=true`), ~~медленный клиент~~ (страница раз в минуту — gap, система живёт), ~~bounded export~~ (2 000 записей NDJSON 469 КБ за 28,7 с, 29 кусков, схема чистая). Открыто: экспорт занимает единственный поток HTTP-сервера на всё время отдачи; вывод C6 с уровнями ESP-IDF — до P6.

**Обновление STM32 — измерено 2026-09-15** (`reports/stm32-update`, плата A, образы `2598fec3…` 1.0.1 и `31628617…` 1.0.0). ~~Загрузка и проверка~~ (1,4 МБ за 185 с, проверка 62,7 с), ~~битый образ~~ (`invalid_image`), ~~понижение~~ (422 без флажка, установка с флажком), ~~обмен~~ (ответ через 55–65 с), ~~самоподтверждение~~ (через 20 мин, сохранилось после сброса), ~~сброс до подтверждения~~ (`rolled_back`, `boot_changed`), ~~IWDG~~ (зависание → сброс через 17,4 с → откат, шаг 0 через шелл). Открыто: потеря питания посреди приёма и обмена, экран против настоящей платы.
...
| `job-manager` | Переходы состояний, прогресс по фазам, дедупликация по ключу, вытеснение истории, 429 при исчерпании | — | ✅ 21 тест |
| `network-manager` | Полная транзакция `staged → committed`/`rolled_back`, валидация IPv4/маски/DNS, таймер подтверждения, health policy на фейковых интерфейсах | DHCP и static на W5500 и C6, снятие кабеля, scan, reconnect, два интерфейса в одной подсети | ✅ 96 тестов + `network_service` 13; hw — DHCP/static/DNS/откат/перезагрузка на W5500 ✅ (`reports/p4`), кабель — владелец, C6 и два интерфейса — до P6 |
...
| `log-store` | Ring wrap, gap, устаревший cursor, смена boot, фильтры, глобальный seq, bounded export, медленный читатель | Burst из реального backend, отсутствие блокировки логгера | ✅ 43 теста (2 конфигурации); привязки — `web_api` (+18 `v1_logs.c`) и `web_api_http` (+4 на настоящем сервере); hw — `reports/p5/hw` |
| `zephyr-log-source` | Форматирование, учёт потерь, отсутствие рекурсии через тот же backend | Ранний boot до старта ring | ✅ 14 тестов; hw — `reports/p5/hw` |
| `coprocessor-manager` | Автомат владения UART, отказ остановки RX, взаимные исключения режимов и операций | EN/BOOT, реальный reset, переключение `console` ↔ `flashing` ↔ `usb_bridge` | ✅ 42 теста + 10 в `network_manager` (крючки claim/release); hw — `reports/p5/hw` |
| `esp32-log-source` | Сборка строк из фрагментов, timeout частичной строки, ANSI strip, невалидный UTF-8, бинарный поток ROM | Реальный вывод C6 на 115200 | ✅ 33 теста, в том числе ROM-поток платы B; hw — ROM C6 платы B на 115200 (`reports/p5/hw`), вывод ESP-IDF с уровнями — до P6 (C6 без прошивки) |
...
| `system-image-store` | Куски и offset, повтор и ремонт сектора, стирание, проверка MCUboot на фикстурах `tests/fixtures/stm32` (верный, битый magic, чужой TLV, обрезанный), истечение — на симуляторе flash и LittleFS | Приём в слот 2 на SPI NOR, обрыв посреди куска | ✅ 76 тестов; hw — приём и проверка на плате A, обрыв — открыт |
| `system-updater` | Фазы, журнал, сверка после старта, откат, самоподтверждение с повтором, `confirm_now`, отказ понижения — на фейковой платформе | Обмен MCUboot, IWDG, сброс до подтверждения | ✅ 64 теста; hw ✅ на плате A (`reports/stm32-update/hw`) |
| Привязки обновления STM32 | `web_api_system`: порядок отказов, одна загрузка на два target, установка и её исходы на фейковой платформе | — | ✅ 21 тест; пробел — установка ESP32, блокирующая STM32 |
| `web-auth` | Параметры KDF, жизненный цикл сессии, expiry, CSRF и Origin, rate limit, атомарность первичного setup | Задержка KDF на реальном CPU под нагрузкой | ✅ 48 тестов; hw ✅ — 0,31 мс/итерация, вход 1,1 с при 3000, система живёт во время вывода (`reports/p2/hw`) |
| `web-api` | Маршрутизация, парсинг и лимиты, коды ошибок, фрагментированный body, abort, неизвестные поля, повтор с тем же ключом | 2–4 клиента одновременно, p95 статусных запросов | ✅ 160 тестов в `web_api` и `web_api_http` (P5: потоковый экспорт, обрыв клиентом, зависший клиент на настоящем сервере, страница по байтам); коды ошибок и парсеры — 24 теста в `api-validation`; hw ✅ измерено — 2 клиента p95 157 мс, 4 клиента p95 313 мс (выше цели) |
| `web-assets` | MIME, ETag, gzip, политика кеша index, SPA fallback против JSON 404 | — | ✅ 9 тестов + сервер в `web_api_http`; генератор — 14 pytest |
...
- Фронтенд: unit-тесты чистых функций, компонентные тесты экранов на фикстурах OpenAPI, e2e против mock-сервера; минимум один прогон пользовательских сценариев против реальной платы перед приёмкой этапа. **P2:** 109 unit/component (vitest; фикстуры типизированы сгенерированными типами и проверяются Ajv по `openapi.json`; словарь сверяется с таблицей ошибок и enum документа; литералы в разметке запрещены проверкой AST), 5 e2e против mock с `--ui` (статика с политикой устройства) и 1 против платы.
...
| отсутствие недокументированных маршрутов | ✅ автоматизировано в P2: `src/web/api/v1/routes.h` сверяется с документом — операции, методы, пути, флаги (session, CSRF, Idempotency-Key, setup token, тело, query) и ресурсы сервера; обслуживается 28 из 35 операций (P5) |
...
CI — решение владельца от 2026-09-12: **GitHub Actions**, `.github/workflows/checks.yml`, на каждый push и pull request, тремя задачами.
...
| `frontend` | `tests/ci/run-frontend-tests.sh` — `npm ci`, типы из `openapi.json`, vitest, сборка с бюджетом 512 КиБ, Playwright против mock со встроенной сборкой; Chromium Playwright. Добавлена в P2, впервые прошла 2026-09-13 (run 34745868566 на `82601a6`, 1 мин 3 с) |
...
| Носитель setup-token | Token отображается в веб-интерфейсе; там же кнопка открытия окна Matter commissioning. ~~**Открыто:** уточнить в P2, token назначения пароля или Matter setup-код~~ **Подтверждено владельцем 2026-09-13:** в веб-интерфейсе показывается именно token назначения пароля. Принятый риск: первый пароль назначит первый, кто откроет страницу в сети |
...
| Watchdog | Позже; строка Boot матрицы раздела 11 остаётся открытой. **2026-09-15, этап «Обновление STM32»: IWDG включён сразу** (решение владельца) — 20 с, кормит кооперативный поток `src/services/system` |
...
Решено в P2, 2026-09-13:

| Вопрос | Решение |
|---|---|
| Стек фронтенда | Выбор оставлен исполнителю («флеш не ограничение, технология, которую знаешь лучше»). Выбраны React, TypeScript, Vite; ограничения раздела 4 выполнены с запасом |
| Setup-token | Показывается в веб-интерфейсе, это token назначения пароля (см. строку выше) |
| Legacy HTTP | Удалить весь: `/upload` на 8080 и все `/api/*` без авторизации. Обновление STM32 по сети недоступно до отдельного этапа (этап «Обновление STM32» начат 2026-09-15, через API v1 с авторизацией) |
| HTTP по IPv6 | Да: сервис на `[::]:80` |

Решено при передаче на P3, 2026-09-13:

| Вопрос | Решение |
|---|---|
| Отладочный код | Не удаляется. Команды шелла Matter (`src/matter/shell.cpp`: окно с печатью QR и ручного кода, число fabrics, удаление всех fabrics) и закомментированный вывод кодов — отладка, а не штатный лог |

Решено в P4, 2026-09-13:

| Вопрос | Решение |
|---|---|
| Где вести P4 | Отдельная ветка `p4-network` и worktree `cedar_p4`: основное дерево занято P3; плата A — P3, плата B — P4 |
| BUS FAULT `w5500_rx` на плате B | Малый патч в `patches/`, применить «в любой момент». Применён; после находки на плате заменён переоткрытием сокета 0 (`patches/README.md`) |
| Строки Wi-Fi матрицы | Открыты до P6: C6 платы B не прошивается — это тестовый случай прошивки из веба |
| Хранилище сетевой конфигурации | Три слота device-config-store — ключи settings-registry |
| Пять циклов питания | Старт короче 30 с считается; на пятом — Ethernet DHCP, DNS автоматически, Wi-Fi выключен с паролем стёртым, SSID сохранён |
| Бэкенд настроек и способ хранения | Решается в отдельном треде; P4 только измеряет (`reports/p4`) |

Решено в P5, 2026-09-14:

| Вопрос | Решение |
|---|---|
| Где вести P5 | Отдельный worktree `cedar_p5`, ветка `p5-logs` от `main` после слияния P4 (`cedar_board/main`, `4582fe4`); плата A — P3, плата B — P5 |
| Сброс C6 через EN из тестов и веба | Разрешён, без записи в память C6 |
| Как включается `usb_bridge` | «Любой способ, дающий меньшее время переключения» — выбор исполнителя: собственный passthrough в coprocessor-manager (узел `uart-bridge0` выключен в overlay приложения), включение открытием USB CDC (DTR) или командой шелла `coproc mode bridge`; операции в API нет, контракт не расширялся |
| `GET /coprocessor/status` | В P5 |
| Ручные шаги на стенде | Отложить в конец или заменить автоматизированными; в P5 ручных шагов нет |

Решено для этапа «Обновление STM32», 2026-09-14 и 2026-09-15 (`reports/stm32-update/README.md`, `reports/p6/stm32-ota-proposal.md`):

| Вопрос | Решение |
|---|---|
| Где делать | Отдельный этап; worktree `cedar_p7`, ветка `p7-stm32-update` от `main` `df852ea` |
| Куда пишется образ | В слот 2 MCUboot (`slot1_partition`, SPI NOR) **сразу**, без промежуточного файла в `/lfs`; MCUboot меняет слоты при старте через scratch 64 КБ |
| Подтверждение новой прошивки | Сама, если проработала 20 минут без падения; плюс консольная команда немедленного подтверждения (`sysupd confirm`). Запись `image_ok` в слот 1 через XIP-драйвер владельца (`FLASH_STM32_OSPI_XIP`, записан в `patches/`), без второго перезапуска |
| Подпись образа | Пока без подписи |
| IWDG | Включить сразу |
| Понижение версии, установка через Wi-Fi, веб-кнопка подтверждения | Не спрашивались, приняты по рекомендации: понижение только с `acknowledge_downgrade`; через Wi-Fi можно; кнопки нет |

Открытые вопросы после P4 — решает владелец:

| Вопрос | Контекст |
|---|---|
| `staged`-кандидат после перезагрузки | Отвечает `404`: кандидат жил в RAM. `410 boot_changed` — только для применённой транзакции, которую откатил старт |
| Счётчик пяти стартов | Считает любой короткий старт — и циклический отказ, и сброс отладчиком; учитывать причину сброса (`hwinfo`) или принять |
| Чтения W5500 «одни единицы» на плате B | Общий SPI2 с C6 без прошивки — гипотеза; патч держит сеть живой ценой до 200 мс ожидания в потоке RX на каждое переоткрытие |

Открытые вопросы после P2 — решает владелец:

| Вопрос | Контекст |
|---|---|
| Восстановление доступа | Забытый пароль или повреждённый verifier блокируют веб навсегда: factory reset вне объёма, settings shell не собран. Варианты: команда шелла по UART (физический доступ), удержание входа на старте, принять |
| Пароль на стендовой плате | Аппаратные проверки P2 оставили администратора с паролем `cedar-bench-P2-changed` (verifier с 3000 итераций) |
| Цена заголовков ответа | Время ответа растёт с числом заголовков; по коду сервер Zephyr отправляет каждое имя, значение и CRLF отдельной записью. Не подтверждено захватом трафика. Варианты: проверить tcpdump, патч в `patches/` и апстрим, сократить заголовки |
| 4 клиента | p95 313 мс при цели 250: сервер однопоточный. Принять для 4, цель — для 2 клиентов, или искать ускорение |
| Слот `DEVICE_CONFIG_SECRET_ADMIN_PASSWORD` | Не используется (verifier в settings-registry, причина — README `web-auth`); удалить при фиксации раскладки в P4 или оставить |
