...
> **Состояние на 2026-09-13.** P0 частично, P1 закрыт, **P2 выполнен локально и
> на плате, закрывается после прогона CI на GitHub.** Готовы: `web-auth`,
...
| P2. Web shell и доступ | ~~Выбор стека и сборка с нуля~~, ~~генерация типов из OpenAPI~~, ~~assets/ETag/gzip~~, ~~login/setup/password/logout~~, ~~общий middleware~~, ~~экран обзора~~ | UI работает офлайн, 2 клиента, нет обхода авторизации через legacy, корректная уборка при abort | P1 | 7–10 | 🟡 **выполнен локально и на плате, закрыть после прогона CI на GitHub** (`reports/p2/README.md`). Критерий на плате: ~~UI офлайн~~ — e2e против платы без внешних запросов и нарушений CSP; ~~2 клиента~~ — 2 браузера (e2e), 2 клиента p95 157 мс без ошибок; ~~нет обхода через legacy~~ — весь legacy HTTP удалён решением владельца, пути отвечают JSON 404, порт 8080 закрыт; ~~уборка при abort~~ — контекст освобождается по ABORTED (sim на настоящем сервере и плата), зависший клиент держит только свой путь 10 с. Также: ~~пятая контрактная проверка~~ — таблица маршрутов устройства сверяется с документом. Не сделано/открыто: workflow с задачей `frontend` не запускался на GitHub; 4 клиента p95 313 мс; гипотеза о цене заголовков в сервере Zephyr не подтверждена захватом; восстановление пароля — вопрос владельцу |
