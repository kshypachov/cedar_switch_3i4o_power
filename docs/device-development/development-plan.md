...
| `job-manager` | Идентификаторы задач, состояния, прогресс, дедупликация, ограниченная история | Конкретные драйверы | **готов** — 21 тест (17 в P1 и 4 на `job_active_ids`/`job_find_by_key` в P2), 5 мутаций |
...
| `web-api` | HTTP маршрутизация, JSON schemas, auth middleware, request contexts, вызовы сервисов | Бизнес-логика и ожидание завершения операций | **готов для P2** — 77 + 14 sim-тестов (ядро и адаптер на настоящем сервере Zephyr); обслуживает 9 из 35 операций, остальные — этапы P3–P6 |
...
| `web-api` | Маршрутизация, парсинг и лимиты, коды ошибок, фрагментированный body, abort, неизвестные поля, повтор с тем же ключом | 2–4 клиента одновременно, p95 статусных запросов | ✅ 77 тестов ядра + 14 адаптера на настоящем сервере; коды ошибок и парсеры — 24 теста в `api-validation`; hw ✅ измерено — 2 клиента p95 157 мс, 4 клиента p95 313 мс (выше цели) |
...
| отсутствие недокументированных маршрутов | ✅ автоматизировано в P2: `src/web/api/v1/routes.h` сверяется с документом — операции, методы, пути, флаги (session, CSRF, Idempotency-Key, setup token, тело, query) и ресурсы сервера; обслуживается 9 из 35 операций |
