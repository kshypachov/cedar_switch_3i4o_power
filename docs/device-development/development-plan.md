...
|---|---|---|---|---:|---|
| P0. Стабильная основа | ~~Удаление записи frontend submodule~~, ~~`esp-serial-flasher` и `tests/esp_loader_integration` на плате~~, ~~таблица разделов C6 с чипа~~, ~~свободное место `/lfs`~~, ~~разбор отказа приложения при старте~~, ~~мультикаст-флуд~~, ~~каркас `tests/` с фейками и CI~~, **устранение голодания планировщика потоком W5500** (решено ждать апстрим, issue #115626 — см. раздел 2), чистый sysbuild отдельно от CLion, 20 холодных/тёплых стартов, состояние rollback в CP firmware | 20 холодных/тёплых стартов, 2 ч network/PSRAM smoke, сохранение существующего I/O; зафиксированы layout C6 и бюджет staging; пустой `sim`-набор проходит в CI | — | 4–6 | 🟡 **частично.** Осталось: чистый sysbuild вне CLion, 20 стартов, rollback в CP firmware |
| P1. Сервисы и контракт | ~~Public APIs модулей~~, ~~Kconfig/CMake~~, ~~config transactions~~, ~~job manager~~, ~~валидация и коды ошибок~~, ~~mock server из OpenAPI~~ | Изоляция модулей, повтор запроса не повторяет действие, mock пригоден для фронтенда | P0 | 4–6 | ✅ **закрыт.** 4 модуля, 124 sim-теста; mock и контрактные проверки, 233 теста; 113 мутаций, все пойманы; CI на каждый коммит. Не входило в этап и остаётся долгом: работа модулей на плате |
...
Проверки самого документа выполняются в CI на каждый коммит:
