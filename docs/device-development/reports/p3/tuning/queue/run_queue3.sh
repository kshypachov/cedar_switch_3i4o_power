...
while IFS="|" read -r backend trial kind build change files cycles; do
  ...
  /Volumes/Programming/Zephyr/zephyr_latest/.venv/bin/python3 $T/run_trial.py --backend $backend --trial $trial --kind $kind \
     --image $build/cedar_switch_3in4out_power/zephyr/zephyr.signed.bin --work $S/work --change "$change" --cycles ${cycles:-5} \
